...
// مسحنا dotenv حيت ماشي محتاجينو فـ Railway
...
const manager = new Discord.ShardingManager('./src/bot.js', {
    totalShards: 'auto',
    token: process.env.DISCORD_TOKEN, // <--- هنا كيقرا من Railway
    respawn: true
});
...
process.on('unhandledRejection', error => {
    ...
    let errMsg = error;
    if (error) if (error.length > 950) errMsg = error.slice(0, 950) + '... view console for details';
    if (error.stack) if (error.stack.length > 950) errMsg = error.stack.slice(0, 950) + '... view console for details';
    ...
    const embed = new Discord.EmbedBuilder()
       ...
       .addFields([
            {
                name: "Error",
                value: errMsg? Discord.codeBlock(errMsg) : "No error",
            },
            {
                name: "Stack error",
